...
// Memoized widget wrapper to prevent unnecessary re-renders
const MemoizedOpenOceanWidget = memo(OpenOceanWidget);

// Component that resets wagmi context so widget uses its own wallet management
const WagmiContextReset = ({ children }: { children: React.ReactNode }) => {
  return (
    <WagmiContext.Provider value={null as any}>
      {children}
    </WagmiContext.Provider>
  );
};

// Static config object - defined outside component to prevent recreation
const staticWidgetConfig: WidgetConfig = {
  integrator: 'DNA',
  variant: 'compact',
  subvariant: 'split',
  appearance: 'dark',
...
export const SwapPage = () => {
  // Delay render to avoid hydration flicker
  ...
  // Create config with stable walletConfig only once
    ...
    <div className="swap-widget mx-auto flex w-full justify-center py-16 px-16 md:py-32 md:px-0">
      <div className="overflow-hidden rounded-2xl surface border border-white/20 w-full max-w-[520px]">
        <div className="p-12 md:p-24">
          {isMounted ? (
            <WagmiContextReset>
              <MemoizedOpenOceanWidget integrator="DNA" config={widgetConfig} />
            </WagmiContextReset>
          ) : (
